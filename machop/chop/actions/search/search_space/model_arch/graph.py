...
from .....passes.graph.transforms.arch import (
    LINEAR_OP,
...
DEFAULT_REDEFINE_LINEAR_CONFIG = {
    "config": {
        "name": "integer",
        "bypass": True,
        "bias_frac_width": 5,
...
class ArchitectureSearchSpace(SearchSpaceBase):
    ...
    def _post_init_setup(self):
        ...
        self.default_config = DEFAULT_REDEFINE_LINEAR_CONFIG
        ...
        match self.config["setup"]["by"]:
            case "name":
                ...
                for n_name, n_info in node_info.items():
                    if n_info["mase_op"] in LINEAR_OP:
                        if n_name in seed:
